...
```{r}
rownames(occur_2_analysis) <- c("Methodology of algorithms","Societal factors","Bias due to data\nvalidation or data security", "Not specified", "Other", "None of the above")
...
ggplot(standard) + geom_col(aes(reorder(standard_daten, N, sum), N),
                              fill = bar_color,
                              width = bar_width) +
  labs(x = "", y = "Number of Participants", title = "Do you think that using standardized data\nwhen training AI algorithms can prevent bias?") +
  scale_y_continuous(breaks = seq(0,200,25)) +
  coord_flip() +
  bar_theme
```
...
*Create table*
```{r}
rownames(prevent_analysis) <- c("Lack of resources","Lack of support from superiors/institution","Lack of knowledge", "Lack of fair data", "Lack of guidelines/recommendations for fair AI", "Other", "Not specified")
...
```{r}
genderfair <-analysis %>% select(fairness_ki, geschlecht, alter)
view(genderfair)
nrow(genderfair)
```
```{r}
...
### create category age group
```{r}
genderfair[,agegroup4:= cut(alter,4,include.lowest=TRUE,
           labels=c("<30","30-40","40-50",">50"))]
genderfair[,table(agegroup4)]
```
...
###column%
```{r}
round(prop.table(table1,2),1)
```

```{r}
genderfair <- subset(genderfair, select = -alter)
```
